...
async function mockFetchGraph(query: string, yearRange: number[], filter: string) {
  ...
  const res = await fetch(`http://35.209.14.138/graph?${params.toString()}`)
...
export default function KnowledgeGraphExplorer3D() {
  ...
  async function handleQA(ev?: React.FormEvent) {
    ...
    const res = await fetch(`http://35.209.14.138/summary?query=${encodeURIComponent(question)}&year=${yearRange[0]}-${yearRange[1]}&filter=${encodeURIComponent(selectedEnvironments.join(','))}`);
